...
@click.pass_context
def list_containers(
    ctx, uid=None, sort: str = "created", reverse: bool = False
) -> None:
    ...
    try:
        with console.status(
            "[bold cyan]Fetching containers...[/bold cyan]", spinner="dots"
        ):
        ...
        for resource in resources:
            table.add_row(
                ...
                f"${resource.cost:,.2f}/hr" if resource.cost else "[dim]—[/dim]",
...
def remove_containers(ctx, container_ids: List[str], yes: bool) -> None:
    ...
    try:
        ...
        async def _delete_all():
            tasks = [
                client.async_serverless.delete_container(cid) for cid in container_ids
            ]
